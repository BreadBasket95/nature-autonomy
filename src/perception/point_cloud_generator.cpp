...
void nature::perception::PointCloudGenerator::toPointCloud2(const std::vector<nature::utils::vec3> &points,
                                                       nature::msg::PointCloud2 &out_point_cloud) {
  int num_lidar_points = static_cast<int>(points.size());
  ...
  for (const nature::utils::vec3 &v : points) {
    for (int i = 0; i < 3; i++) {
      dataUnion u;
      u.f = v[i];
      for (int j = 0; j < static_cast<int>(sizeof(float)); j++) {
...
void nature::perception::PointCloudGenerator::toPointCloud2(const std::vector<nature::utils::vec3> &points,
                                                       const std::vector<int> &seg_values,
                                                       nature::msg::PointCloud2 &out_point_cloud) {
  int num_lidar_points = static_cast<int>(points.size());
  out_point_cloud.height = 1;
  out_point_cloud.width = num_lidar_points;
  ...
  for (int v_i = 0; v_i < static_cast<int>(points.size()); v_i++) {
    nature::utils::vec3 v = points[v_i];
    float seg_value = static_cast<float>(seg_values[v_i]);
    for (int i = 0; i < 3; i++) {
      dataUnion u;
      u.f = v[i];
      for (int j = 0; j < static_cast<int>(sizeof(float)); j++) {
